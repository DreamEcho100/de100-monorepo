...
export default function FilesPage() {
	...
	const filesClient = createMemo(() => createOrpcFilesClient());
	const [isHydrated, setIsHydrated] = createSignal(false);
	const [uploadError, setUploadError] = createSignal<string | null>(null);
	const [uploadNotice, setUploadNotice] = createSignal<string | null>(null);
	const [signedAccessFileId, setSignedAccessFileId] = createSignal<string | null>(null);
	...
	const canLoadFiles = () => isHydrated() && !session().isPending && !!session().data;
	...
	const completeUploadMutation = createMutation(() =>
		orpc.files.completeUpload.mutationOptions({
			onError: (error) => {
				setUploadError(localizeOrpcError(error, t) ?? t("files.status.completeFailed"));
			},
			onSuccess: async () => {
				setUploadNotice(t("files.status.uploadCompletedNotice"));
				await refreshFiles();
			},
		}),
	);
	const deleteFileMutation = createMutation(() =>
		orpc.files.delete.mutationOptions({
			onError: (error) => {
				setUploadError(localizeOrpcError(error, t) ?? t("files.status.deleteFailed"));
			},
			onSuccess: async () => {
				setUploadNotice(t("files.status.deletedNotice"));
				await refreshFiles();
			},
		}),
	);
	const issueSignedAccessMutation = createMutation(() =>
		orpc.files.issueSignedAccess.mutationOptions({
			onError: (error) => {
				setUploadError(localizeOrpcError(error, t) ?? t("files.status.signedAccessFailed"));
			},
			onSuccess: (result, variables) => {
				setSignedAccessLink(result?.url ?? null);
				setSignedAccessFileId(variables.id);
				setUploadNotice(t("files.status.signedAccessReady"));
			},
		}),
	);
						...
						<CardContent class="space-y-4">
							...
							<Show when={canLoadFiles() && !filesItems.isPending && !filesItems.isError}>
								<FilesList
									...
									signedAccessFileId={signedAccessFileId()}
