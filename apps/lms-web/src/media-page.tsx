        ...
        {(item) => (
          <li class="space-y-3 rounded-xl border border-border/70 px-4 py-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
              ...
              <div class="flex flex-wrap gap-2">
                <Badge variant="outline">
                  {props.t(`media.status.badges.${item.status}`)}
                </Badge>
                <Badge
                  variant={
                    item.visibility === "public" ? "secondary" : "outline"
                  }
                >
            ...
            <div class="flex flex-wrap gap-3">
              ...
              <Show
                when={props.supportsSignedDelivery && item.status === "ready"}
              >
                <Button
                ...
                >
                  <Show
                    fallback={
                      <span>{props.t("media.actions.generateSignedUrl")}</span>
                    }
              ...
              <Show
                when={
                  props.signedAccessMediaId === item.id &&
                  props.signedAccessLink
                }
              >
                {(link) => (
                  <a
              ...
              <Show when={item.status === "draft"}>
                ...
                >
                  <Show
                    fallback={
                      <span>{props.t("media.actions.confirmUpload")}</span>
                    }
              ...
              >
                <Show
                  ...
                  when={props.isDeletingMedia(item.id)}
                >
                  <Loader2 class="animate-spin" size={18} />
                </Show>
              </Button>
...
export default function MediaPage() {
  ...
  const [signedAccessLink, setSignedAccessLink] = createSignal<string | null>(
    null,
  );
  const [signedAccessMediaId, setSignedAccessMediaId] = createSignal<
    string | null
  >(null);
  ...
  const canLoadMedia = () =>
    isHydrated() && !session().isPending && !!session().data;
    ...
    orpc.media.issueSignedAccess.mutationOptions({
      onError: (error) => {
        setUploadError(
          error instanceof Error
            ? error.message
            : t("media.status.signedAccessFailed"),
        );
      },
  ...
  const draftMedia = createMemo(() =>
    ((mediaItems.data ?? []) as MediaRecord[]).filter(
  ...
  function isConfirmingMedia(id: string) {
    return (
      ...
      confirmUploadMutation.variables?.id === id
    );
  }

  function isDeletingMedia(id: string) {
    return (
      deleteMediaMutation.isPending && deleteMediaMutation.variables?.id === id
    );
  }

  function isIssuingSignedAccess(id: string) {
    return (
      issueSignedAccessMutation.isPending &&
      issueSignedAccessMutation.variables?.id === id
    );
  }
          ...
          <>
            <Card class="border-border/70 bg-card/95 shadow-black/5 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  {t("media.sections.backendDescription")}
                </CardDescription>
              </CardHeader>
            ...
            <Card class="border-border/70 bg-card/95 shadow-black/5 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  {t("media.sections.uploadDescription")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form class="grid gap-4" onSubmit={handleSubmit}>
                  <Field class="grid gap-4">
                    <FieldLabel for="media-file">
                      {t("media.fields.file")}
                    </FieldLabel>
                    <FieldContent>
                      <Input
                      ...
                      />
                      <FieldDescription>
                        {t("media.fields.visibilityDescription")}
                      </FieldDescription>
                    </FieldContent>
                  </Field>

                  <Field class="grid gap-4">
                    <FieldLabel for="media-visibility">
                      {t("media.fields.visibility")}
                    </FieldLabel>
                    <FieldContent>
                      <select
                        ...
                        onChange={(changeEvent) =>
                          setVisibility(
                            changeEvent.currentTarget.value === "public"
                              ? "public"
                              : "private",
                          )
                        }
                  ...
                  <Button
                    disabled={!file() || uploadMediaMutation.isPending}
                    type="submit"
                  >
                    <Show
              ...
              <CardHeader>
                <div class="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>
                      {t("media.sections.draftsDescription")}
                    </CardDescription>
                  </div>
                  <Badge variant="outline">
              ...
              <CardContent class="space-y-4">
                ...
                >
                  <MediaList
                    ...
                    supportsSignedDelivery={
                      !!mediaCapabilities.data?.supportsSignedDelivery
                    }
                    t={t}
                  />
                </Show>
              ...
              <CardHeader>
                <div class="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>
                      {t("media.sections.readyDescription")}
                    </CardDescription>
                  </div>
                  <Badge variant="secondary">
