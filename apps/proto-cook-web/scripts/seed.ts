...
type DatabaseClient = Awaited<ReturnType<typeof import("@de100/apps-proto-cook-db")["createDb"]>>;
	...
	switch (fixture.contentType) {
		...
		case "image/svg+xml":
			return [
				...
				'<text x="48" y="112" fill="#0f172a" font-family="system-ui, sans-serif" font-size="24" font-weight="700">Proto Cook seeded files fixture</text>',
		...
		default:
			return [
				"Proto Cook seeded files fixture",
...
async function ensureSeedUser(
	db: DatabaseClient,
	auth: ReturnType<typeof import("@de100/apps-proto-cook-auth")["createAuth"]>,
	fixtures: SeedUserFixture,
) {
	const { account, user } = await import("@de100/apps-proto-cook-db/schema/auth");
...
async function syncSeedTodos(db: DatabaseClient, userId: string, fixtures: SeedTodoFixture[]) {
	...
	const { todo } = await import("@de100/apps-proto-cook-db/schema/todo");
...
async function syncSeedFiles(db: DatabaseClient, userId: string, fixtures: SeedFilesFixture[]) {
	const { inArray } = await import("drizzle-orm");
	const { files } = await import("@de100/apps-proto-cook-db/schema/files");
	const { getConfiguredFilesBucket } = await import("@de100/apps-proto-cook-api/files-storage");
...
async function main() {
	...
	const [{ createAuth }, { createDb }, { getConfiguredFilesStorageDriver }] = await Promise.all([
		import("@de100/apps-proto-cook-auth"),
		import("@de100/apps-proto-cook-db"),
		import("@de100/apps-proto-cook-api/files-storage"),
	]);
	...
	try {
		...
		if (filesStorageDriver === "local") {
			console.log(
				`Seeded ${writtenFilesObjects} local files objects under ${process.env.APP_PROTO_COOK_FILES_LOCAL_ROOT || "./.local/files"}.`,
			);
		} else {
			console.log(
