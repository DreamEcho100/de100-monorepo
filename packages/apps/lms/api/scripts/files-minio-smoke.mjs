...
const endpoint = process.env.APP_LMS_FILES_S3_ENDPOINT ?? "http://127.0.0.1:9000";
const region = process.env.APP_LMS_FILES_S3_REGION ?? "us-east-1";
const accessKeyId = process.env.APP_LMS_FILES_S3_ACCESS_KEY_ID ?? "minioadmin";
const secretAccessKey = process.env.APP_LMS_FILES_S3_SECRET_ACCESS_KEY ?? "minioadmin";
const publicBucket = process.env.APP_LMS_FILES_S3_PUBLIC_BUCKET ?? "public-files";
const privateBucket = process.env.APP_LMS_FILES_S3_PRIVATE_BUCKET ?? "private-files";
const keepTemp = process.env.APP_LMS_FILES_MINIO_SMOKE_KEEP_TMP === "true";
...
const workDir = join(tmpdir(), `de100-lms-files-minio-smoke-${runId}`);
