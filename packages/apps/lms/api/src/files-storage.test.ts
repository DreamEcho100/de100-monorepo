...
function stubS3Env(provider: "minio" | "r2") {
	vi.stubEnv("APP_LMS_FILES_STORAGE_DRIVER", "s3");
	vi.stubEnv("APP_LMS_FILES_S3_PROVIDER", provider);
	vi.stubEnv("APP_LMS_FILES_S3_ENDPOINT", "http://127.0.0.1:9000");
	vi.stubEnv("APP_LMS_FILES_S3_REGION", provider === "r2" ? "auto" : "us-east-1");
	vi.stubEnv("APP_LMS_FILES_S3_ACCESS_KEY_ID", "access_key");
	vi.stubEnv("APP_LMS_FILES_S3_SECRET_ACCESS_KEY", "secret_key");
	vi.stubEnv("APP_LMS_FILES_S3_PUBLIC_BUCKET", "lms-public-files");
	vi.stubEnv("APP_LMS_FILES_S3_PRIVATE_BUCKET", "lms-private-files");
	vi.stubEnv("APP_LMS_FILES_S3_FORCE_PATH_STYLE", "true");
}

describe("LMS files storage upload targets", () => {
	beforeEach(() => {
	...
	it("maps local and S3-compatible env profiles to planner backends", () => {
		vi.stubEnv("APP_LMS_FILES_STORAGE_DRIVER", "local");
