...
- `pnpm -F @de100/apps-lms-infra selfhost:preflight`: validate env and mode wiring.
- `pnpm -F @de100/apps-lms-infra selfhost:health`: hit the configured health endpoint.
- `pnpm -F @de100/apps-lms-infra selfhost:smoke:public`: validate key public routes and OpenAPI endpoint behavior.
- `pnpm -F @de100/apps-lms-infra selfhost:smoke:hosted -- --url <origin> --env <environment>`: run hosted smoke orchestration (`verify:full`) and scaffold evidence in one command.
- `pnpm -F @de100/apps-lms-infra selfhost:verify`: run preflight and health checks together.
- `pnpm -F @de100/apps-lms-infra selfhost:verify:full`: run verify plus public smoke checks.
- `pnpm -F @de100/apps-lms-infra selfhost:evidence:init -- <environment>`: scaffold hosted smoke evidence from template.
- `pnpm -F @de100/apps-lms-infra destroy`: print controlled destroy guidance.
...
4. Run `pnpm -F @de100/apps-lms-infra selfhost:smoke:hosted -- --url https://your-app-domain.example --env <environment>`.
