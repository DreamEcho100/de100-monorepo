...
import { runLmsFilesWorkerLoop, runLmsFilesWorkerOnce } from "./runner";
...
const request = new Request("http://localhost/internal/files-worker") as unknown as NonNullable<
	Parameters<typeof runLmsFilesWorkerOnce>[0]["request"]
>;

describe("LMS files worker runner", () => {
	it("runs one queued job through the injected processing pipeline", async () => {
		...
		const result = await runLmsFilesWorkerOnce({
			db: {} as DbInstance,
			pipeline,
			queue,
			repositories,
