...
type ProtoCookFilesContext = FilesRequestContext<{ db: DbInstance }>;

type ProcessProtoCookUploadedFileInput = {
	file: FileRecord;
	filesContext: ProtoCookFilesContext;
...
type CreateProtoCookFilesProcessingPipelineOptions = {
	dependencies?: FilesProcessingDependencyRegistry;
	repositories?: ProtoCookFilesRepositories;
	storageProvider?: FilesStorageProvider;
};

export async function processProtoCookUploadedFile(input: ProcessProtoCookUploadedFileInput) {
	const result = await runFilesProcessingJob({
		...
		pipeline:
			input.pipeline ??
			createProtoCookFilesProcessingPipeline(input.filesContext, {
...
export function createProtoCookFilesProcessingPipeline(
	filesContext: ProtoCookFilesContext,
	options: CreateProtoCookFilesProcessingPipelineOptions = {},
		...
		},
		{
			...
			async run(input) {
				if (!["audio", "image", "video"].includes(input.file.kind)) {
					return {
						reason: "This file kind does not have a configured Proto Cook variant generator.",
						status: "skipped",
					};
				}
...
async function createVideoHlsArtifacts(input: {
	...
	if (!inputFormat) {
		return {
			...
			reason: "This video input format is not configured for Proto Cook HLS processing.",
			status: "skipped" as const,
		};
	}
