...
import type { ProtoCookCourseFilesRepository, ProtoCookCourseRecord } from "../files-course";
import {
	attachProtoCookCourseVideoFile,
	canManageProtoCookCourse,
	courseEnrollmentStatusValues,
	courseLessonVisibilityValues,
	courseStatusValues,
	createProtoCookCourseFilesRepository,
...
const courseLessonOutputSchema = z.object({
	...
	title: z.string().min(1),
	updatedAt: z.date(),
	visibility: z.enum(courseLessonVisibilityValues),
});
...
const courseEnrollmentOutputSchema = z.object({
	...
	id: z.string().min(1),
	metadata: z.record(z.string(), z.unknown()).nullable(),
	status: z.enum(courseEnrollmentStatusValues),
	updatedAt: z.date(),
...
const createLessonInputSchema = z.object({
	...
	slug: z.string().min(1),
	title: z.string().min(1),
	visibility: z.enum(courseLessonVisibilityValues).optional(),
});

const courseSlugInputSchema = z.object({
	courseSlug: z.string().min(1),
	metadata: metadataSchema,
	status: z.enum(courseEnrollmentStatusValues).optional(),
});
...
export const coursesRouter = {
	attachLessonVideo: protectedProcedure
		...
		.handler(async ({ context, input }) => {
			const repository = createProtoCookCourseFilesRepository(context.db);
			...
			return attachProtoCookCourseVideoFile({
				auth: { userId: context.session.user.id },
				courseId: resolvedAccessContext.course.id,
				fileId: input.fileId,
				files: createProtoCookFilesRepositories(context.db),
	...
	createCourse: protectedProcedure
		...
		.route({
			...
			summary: "Create an Proto Cook course owned by the signed-in user",
			tags: ["Courses"],
		})
	...
	createPlaybackSession: publicProcedure
		...
		.handler(async ({ context, input }) => {
			...
			const resolvedAccessContext = requireCourseAccessContext(accessContext);
			const files = createProtoCookFilesRepositories(context.db);

			const result = await createProtoCookCourseLessonPlaybackSession({
	...
	enrollCurrentUser: protectedProcedure
		...
		.handler(async ({ context, input }) => {
			const repository = createProtoCookCourseFilesRepository(context.db);
			const course = await requireCourseBySlug(repository, input.courseSlug);
			const canManage = canManageProtoCookCourse({
...
async function requireCourseBySlug(repository: ProtoCookCourseFilesRepository, slug: string) {
...
function requireCanManageCourse(input: { course: ProtoCookCourseRecord; userId: string }) {
	if (
		!canManageProtoCookCourse({
