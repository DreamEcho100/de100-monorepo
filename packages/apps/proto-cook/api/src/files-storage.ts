...
export class FilesBindingsUnavailableError extends FilesStorageUnavailableError {
	constructor() {
		super(
			"Files storage is unavailable for this request. Configure runtime bucket bindings or APP_PROTO_COOK_FILES_S3_* values, or switch APP_PROTO_COOK_FILES_STORAGE_DRIVER to local.",
		);
	}
}
...
export class FilesLocalStorageUnavailableError extends FilesStorageUnavailableError {
	constructor() {
		super(
			"Local files storage is unavailable in this runtime. Use the Node-based local dev server or switch APP_PROTO_COOK_FILES_STORAGE_DRIVER back to s3.",
		);
	}
}
...
function getConfiguredFilesS3Config(): FilesS3Config {
	...
	const forcePathStyleOverride = parseRuntimeBoolean(
		getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_FORCE_PATH_STYLE"),
	);

	return {
		accessKeyId:
			getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_ACCESS_KEY_ID") ??
			configuredS3Values?.accessKeyId,
		endpoint:
			getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_ENDPOINT") ?? configuredS3Values?.endpoint,
		forcePathStyle: forcePathStyleOverride ?? configuredS3Values?.forcePathStyle ?? true,
		privateBucket:
			getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_PRIVATE_BUCKET") ??
			configuredS3Values?.privateBucket,
		provider:
			(getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_PROVIDER") as FilesS3Provider | undefined) ??
			configuredS3Values?.provider ??
			"r2",
		publicBucket:
			getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_PUBLIC_BUCKET") ??
			configuredS3Values?.publicBucket,
		region:
			getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_REGION") ?? configuredS3Values?.region ?? "auto",
		secretAccessKey:
			getRuntimeEnvValue("APP_PROTO_COOK_FILES_S3_SECRET_ACCESS_KEY") ??
...
export function getConfiguredFilesStorageDriver(): FilesStorageDriver {
	const runtimeDriver = process.env.APP_PROTO_COOK_FILES_STORAGE_DRIVER;
