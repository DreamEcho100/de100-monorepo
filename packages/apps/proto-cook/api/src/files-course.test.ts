...
import {
	attachProtoCookCourseVideoFile,
	canManageProtoCookCourse,
	createProtoCookCourseLessonPlaybackSession,
	resolveProtoCookCourseLessonAccess,
	selectProtoCookCourseVideoAssetStatus,
	serializeProtoCookCourseChapterRecord,
	serializeProtoCookCourseEnrollmentRecord,
	serializeProtoCookCourseLessonRecord,
	serializeProtoCookCourseRecord,
	serializeProtoCookCourseVideoAssetRecord,
	shouldQueueProtoCookCourseVideoProcessing,
} from "./files-course";
import type { ProtoCookFilesRepositories } from "./files-repositories";

const now = new Date("2026-06-02T08:00:00.000Z");

describe("Proto Cook course files policy", () => {
	...
	it("maps uploaded file status to course video asset processing decisions", () => {
		expect(selectProtoCookCourseVideoAssetStatus("stored")).toBe("processing");
		expect(selectProtoCookCourseVideoAssetStatus("ready")).toBe("processing");
		expect(selectProtoCookCourseVideoAssetStatus("draft")).toBe("draft");
		expect(selectProtoCookCourseVideoAssetStatus("deleted")).toBe("failed");
		expect(shouldQueueProtoCookCourseVideoProcessing("stored")).toBe(true);
		expect(shouldQueueProtoCookCourseVideoProcessing("ready")).toBe(true);
		expect(shouldQueueProtoCookCourseVideoProcessing("draft")).toBe(false);
	});
});

describe("Proto Cook course files workflows", () => {
	...
	it("creates signed HLS playback sessions only for entitled ready assets", async () => {
		...
		files.artifacts = {
			...
			playbackSessions,
		} as ProtoCookFilesRepositories["artifacts"];
...
describe("Proto Cook course files serializers", () => {
...
function createFakeFilesRepositories(file: FileRecord): ProtoCookFilesRepositories {
	return {
		artifacts: {
			groups: {
				createGroup: vi.fn(),
