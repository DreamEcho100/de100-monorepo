...
import { DEFAULT_APP_PROTO_COOK_CACHE_DRIVER } from "./types";

type CacheRegistry = Map<string, CacheClient>;

type GlobalCacheState = typeof globalThis & {
	__de100AppsProtoCookCacheRegistry?: CacheRegistry;
};

function createCacheRegistryKey(config: CacheClientConfig) {
	return JSON.stringify({
		driver: config.driver,
		keyPrefix: config.keyPrefix ?? DEFAULT_APP_PROTO_COOK_CACHE_DRIVER,
...
function getCacheRegistry() {
	...
	if (!globalCacheState.__de100AppsProtoCookCacheRegistry) {
		globalCacheState.__de100AppsProtoCookCacheRegistry = new Map<string, CacheClient>();
	}

	return globalCacheState.__de100AppsProtoCookCacheRegistry;
}

function requireRedisUrl(redisUrl: string | undefined) {
	if (!redisUrl) {
		throw new Error("REDIS_URL is required when APP_PROTO_COOK_CACHE_DRIVER=redis.");
	}
...
function requireUpstashConfig(config: CacheClientConfig) {
	if (!config.upstashRedisUrl) {
		throw new Error(
			"APP_PROTO_COOK_UPSTASH_REDIS_URL is required when APP_PROTO_COOK_CACHE_DRIVER=upstash.",
		);
	}

	if (!config.upstashRedisToken) {
		throw new Error(
			"APP_PROTO_COOK_UPSTASH_REDIS_TOKEN is required when APP_PROTO_COOK_CACHE_DRIVER=upstash.",
		);
	}
...
export function createCacheClient(config: CacheClientConfig): CacheClient {
	const keyPrefix = config.keyPrefix ?? DEFAULT_APP_PROTO_COOK_CACHE_DRIVER;
