...
function printUsage() {
	console.log(
		"Usage: pnpm -F @de100/apps-proto-cook-infra selfhost:smoke:hosted -- --url <origin> [--env <environment>] [--timeout-ms <ms>] [--health-url <url>] [--skip-evidence-init] [--evidence-dry-run] [--force-evidence]",
	);
	console.log("Examples:");
	console.log(
		"  pnpm -F @de100/apps-proto-cook-infra selfhost:smoke:hosted -- --url https://app.example --env production",
	);
	console.log(
		"  pnpm -F @de100/apps-proto-cook-infra selfhost:smoke:hosted -- --url https://staging.example --env staging --evidence-dry-run",
	);
}
...
function parseOptions(rawArgs) {
	const options = {
		baseUrl: process.env.APP_PROTO_COOK_SMOKE_BASE_URL,
		evidenceDryRun: false,
		environment: process.env.APP_PROTO_COOK_EVIDENCE_ENV || "staging",
		forceEvidence: false,
		healthUrl: process.env.APP_PROTO_COOK_HEALTHCHECK_URL,
		skipEvidenceInit: false,
		timeoutMs: process.env.APP_PROTO_COOK_SMOKE_TIMEOUT_MS || "15000",
	};
	...
	if (!options.baseUrl) {
		throw new Error(
			"Missing base URL. Provide --url <origin> or set APP_PROTO_COOK_SMOKE_BASE_URL.",
		);
	}
...
process.env.APP_PROTO_COOK_SMOKE_BASE_URL = options.baseUrl;
process.env.APP_PROTO_COOK_SMOKE_TIMEOUT_MS = String(options.timeoutMs);
process.env.APP_PROTO_COOK_HEALTHCHECK_URL = options.healthUrl;
process.env.APP_PROTO_COOK_EVIDENCE_ENV = options.environment;
