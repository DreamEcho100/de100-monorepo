...
function resolveBaseUrl() {
	const explicitBaseUrl =
		getEnvValue("APP_PROTO_COOK_SMOKE_BASE_URL") ??
		getEnvValue("VITE_APP_PROTO_COOK_SERVER_URL") ??
		getEnvValue("APP_PROTO_COOK_CORS_ORIGIN");
	if (explicitBaseUrl) {
		return explicitBaseUrl;
	}

	const healthcheckUrl = getEnvValue("APP_PROTO_COOK_HEALTHCHECK_URL");
	if (healthcheckUrl) {
		return new URL(healthcheckUrl).origin;
	}

	const serverPort = getEnvValue("APP_PROTO_COOK_SERVER_PORT") || "3000";
...
const timeoutMs = Number.parseInt(getEnvValue("APP_PROTO_COOK_SMOKE_TIMEOUT_MS") || "15000", 10);
...
/** @type {Array<Awaited<ReturnType<typeof runCheck>>>} */
const results = [];
