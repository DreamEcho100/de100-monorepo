...
- Proto Cook app service on internal Docker network
...
services:
	...
	proto-cook-web:
		...
		env_file:
			- ./.env.deploy.local
		command: >-
			sh -lc "corepack enable && pnpm install --frozen-lockfile && pnpm -F @de100/apps-proto-cook-web build && pnpm -F @de100/apps-proto-cook-web start"
		volumes:
			- /opt/proto-cook/repo:/workspace
		labels:
			- traefik.enable=true
			- traefik.http.routers.proto-cook.rule=Host(`your-app-domain.example`)
			- traefik.http.routers.proto-cook.entrypoints=websecure
			- traefik.http.routers.proto-cook.tls.certresolver=le
			- traefik.http.services.proto-cook.loadbalancer.server.port=3000
		restart: unless-stopped
```
