...
1. Create one Coolify project for Proto Cook.
2. Register the repo and branch.
3. Restrict deployment access to maintainers only.

## 3. Configure App Service In Coolify

Use one app service for Proto Cook web runtime:

- Port: `3000` (or your `APP_PROTO_COOK_SERVER_PORT` value)
- Build command: `pnpm install --frozen-lockfile && pnpm -F @de100/apps-proto-cook-web build`
- Start command: `pnpm -F @de100/apps-proto-cook-web start`
- Health path: `/health`
...
3. For `APP_PROTO_COOK_FILES_STORAGE_DRIVER=s3`, include `APP_PROTO_COOK_FILES_S3_PROVIDER` and all required `APP_PROTO_COOK_FILES_S3_*` values.

## 4. Add Caddy As Edge Reverse Proxy

Run Caddy on the host or in a dedicated service that forwards to the Proto Cook app container.
