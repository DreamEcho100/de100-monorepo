...
```bash
export APP_PROTO_COOK_SMOKE_BASE_URL="https://your-app-domain.example"
export APP_PROTO_COOK_SMOKE_TIMEOUT_MS="15000"
```

3. If health endpoint differs from default, set:

```bash
export APP_PROTO_COOK_HEALTHCHECK_URL="https://your-app-domain.example/health"
```
...
2. If `selfhost:health` fails:
   - verify reverse proxy routing and `APP_PROTO_COOK_HEALTHCHECK_URL`
...
Focus on auth flows, files upload/read, and signed URL behavior.
...
```bash
pnpm -F @de100/apps-proto-cook-infra selfhost:evidence:init -- <environment>
```
