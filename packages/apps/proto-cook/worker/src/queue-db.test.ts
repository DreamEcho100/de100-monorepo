...
import type { ProtoCookFilesWorkerJobQueueStore } from "./queue-db";
import {
	createProtoCookFilesDbPollingQueueAdapter,
	createWorkerPayloadFromProcessingJob,
} from "./queue-db";

const now = new Date("2026-06-07T10:00:00.000Z");

describe("Proto Cook DB-polling files queue", () => {
	it("enqueues, claims due jobs, and marks completion", async () => {
		const store = createInMemoryQueueStore();
		const queue = createProtoCookFilesDbPollingQueueAdapter(store, { now: () => now });
...
function createInMemoryQueueStore(): ProtoCookFilesWorkerJobQueueStore & {
