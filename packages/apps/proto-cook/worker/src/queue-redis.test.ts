...
import type { ProtoCookFilesRedisQueueClient } from "./queue-redis";
import { createProtoCookFilesRedisQueueAdapter } from "./queue-redis";

const now = new Date("2026-06-07T10:00:00.000Z");

describe("Proto Cook Redis files queue", () => {
	it("pushes ready jobs and returns them FIFO through Redis list order", async () => {
		const client = createFakeRedisClient();
		const queue = createProtoCookFilesRedisQueueAdapter({
			client,
			keyPrefix: "files",
			now: () => now,
		});
...
function createFakeRedisClient(): ProtoCookFilesRedisQueueClient & {
