...
export type ProtoCookFilesRedisQueueClient = {
...
export type ProtoCookFilesRedisQueueAdapterOptions = {
	client: ProtoCookFilesRedisQueueClient;
	keyPrefix?: string;
	now?: () => Date;
	promoteLimit?: number;
};

export function createProtoCookFilesRedisQueueAdapter(
	options: ProtoCookFilesRedisQueueAdapterOptions,
): FilesQueueAdapter {
	const keyPrefix = normalizeKeyPrefix(options.keyPrefix ?? "de100:proto-cook:files");
