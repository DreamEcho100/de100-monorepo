...
- object storage when `APP_PROTO_COOK_FILES_STORAGE_DRIVER=s3`

Recommended production stack:

- Coolify plus Caddy for app runtime and edge routing
- Neon for `APP_PROTO_COOK_DATABASE_URL`
- Upstash for `APP_PROTO_COOK_CACHE_DRIVER=upstash`
- Resend for `APP_PROTO_COOK_EMAIL_DRIVER=resend`
- S3-compatible object storage for files when `APP_PROTO_COOK_FILES_STORAGE_DRIVER=s3`

Supported alternatives already in the codebase:

- standard Postgres instead of Neon
- `APP_PROTO_COOK_CACHE_DRIVER=redis` instead of Upstash
- `APP_PROTO_COOK_CACHE_DRIVER=memory` for single-instance or low-stakes environments
- `APP_PROTO_COOK_FILES_STORAGE_DRIVER=local` for local-only or non-shared deployments
...
4. `APP_PROTO_COOK_BETTER_AUTH_SECRET` with at least 32 characters.
...
- Hetzner: `packages/apps/proto-cook/infra/docs/providers/hetzner.md`
- DigitalOcean: `packages/apps/proto-cook/infra/docs/providers/digitalocean.md`
- Hostinger: `packages/apps/proto-cook/infra/docs/providers/hostinger.md`
...
APP_PROTO_COOK_DATABASE_URL=postgresql://user:password@your-db-host/dbname
...
If you use `APP_PROTO_COOK_FILES_STORAGE_DRIVER=local`, replace S3 variables with:
...
```env
APP_PROTO_COOK_HEALTHCHECK_URL=https://your-app-domain.example/health
APP_PROTO_COOK_HEALTHCHECK_TIMEOUT_MS=10000
```

If `APP_PROTO_COOK_FILES_SIGNING_SECRET` is unset, the app falls back to `APP_PROTO_COOK_BETTER_AUTH_SECRET`.
...
- Coolify primary: `packages/apps/proto-cook/infra/docs/coolify-deployment/setup-from-scratch.md`
- Traefik alternative: `packages/apps/proto-cook/infra/docs/traefik-deployment/service-setup.md`

5. Run post-deploy checks.

```bash
pnpm -F @de100/apps-proto-cook-infra selfhost:smoke:hosted -- --url https://your-app-domain.example --env <environment>
```
...
- `packages/apps/proto-cook/infra/docs/checklists/hosted-smoke-run-procedure.md`
- `packages/apps/proto-cook/infra/docs/checklists/post-deploy-smoke-checklist.md`
- `pnpm -F @de100/apps-proto-cook-infra selfhost:evidence:init -- <environment>` (only when manual sequence is used, or when `--skip-evidence-init` was passed)
- `docs/evidence/templates/hosted-deploy-smoke-template.md`

`pnpm -F @de100/apps-proto-cook-infra deploy` is an alias for `selfhost:verify`.

Optional smoke URL override:

```env
APP_PROTO_COOK_SMOKE_BASE_URL=https://your-app-domain.example
...
- `APP_PROTO_COOK_BETTER_AUTH_URL` must be the final public origin plus `/api/auth`.
- `APP_PROTO_COOK_CORS_ORIGIN` must match the browser origin that serves the app.
- `VITE_APP_PROTO_COOK_SERVER_URL` should stay empty for same-origin deployments unless browser and server origins are intentionally split.
...
- `APP_PROTO_COOK_FILES_S3_PROVIDER`
- `APP_PROTO_COOK_FILES_S3_ENDPOINT`
- `APP_PROTO_COOK_FILES_S3_PUBLIC_BUCKET`
- `APP_PROTO_COOK_FILES_S3_PRIVATE_BUCKET`
- `APP_PROTO_COOK_FILES_S3_ACCESS_KEY_ID` and `APP_PROTO_COOK_FILES_S3_SECRET_ACCESS_KEY` together, if either is set

Runtime resolution order:

- use request runtime bucket bindings when present
- otherwise use `APP_PROTO_COOK_FILES_S3_*` values for hosted S3-compatible access

Use `APP_PROTO_COOK_FILES_STORAGE_DRIVER=local` only for local or single-node flows where shared object storage is not required.

## Verification after deploy

Treat this checklist as `implemented-and-unverified` until matching evidence is added under `docs/evidence/`.

Use `packages/apps/proto-cook/infra/docs/checklists/post-deploy-smoke-checklist.md` as the execution checklist and store results with `docs/evidence/templates/hosted-deploy-smoke-template.md`.
For a step-by-step operator flow, use `packages/apps/proto-cook/infra/docs/checklists/hosted-smoke-run-procedure.md`.
...
- Re-run `pnpm -F @de100/apps-proto-cook-infra selfhost:preflight` and resolve reported missing variables.
...
- Verify `APP_PROTO_COOK_SMOKE_BASE_URL` only points to the intended hosted target for smoke runs.

### Health check fails

- Set `APP_PROTO_COOK_HEALTHCHECK_URL` to the deployed app URL.
- Confirm reverse proxy routes `/health` to the app service.
- Verify `APP_PROTO_COOK_SERVER_PORT` matches the container port exposed by your orchestrator.

### Auth works locally but fails after deploy

Check these first:

- `APP_PROTO_COOK_BETTER_AUTH_URL`
- `APP_PROTO_COOK_CORS_ORIGIN`
- `APP_PROTO_COOK_BETTER_AUTH_SECRET`
...
- `APP_PROTO_COOK_FILES_STORAGE_DRIVER`
...
Check active `APP_PROTO_COOK_CACHE_DRIVER` and matching vars:

- `REDIS_URL` for `redis`
- `APP_PROTO_COOK_UPSTASH_REDIS_URL` and `APP_PROTO_COOK_UPSTASH_REDIS_TOKEN` for `upstash`

If you do not want a shared cache backend yet, switch to `APP_PROTO_COOK_CACHE_DRIVER=memory`.
...
- `APP_PROTO_COOK_EMAIL_DRIVER=resend`
- `APP_PROTO_COOK_EMAIL_FROM`
- `APP_PROTO_COOK_RESEND_API_KEY`

For local development, switch back to `APP_PROTO_COOK_EMAIL_DRIVER=log` and confirm generated Better Auth links appear in local server logs.
...
- `packages/apps/proto-cook/infra/docs/README.md`
- `packages/apps/proto-cook/infra/docs/runbooks/migration-cutover.md`
- `packages/apps/proto-cook/infra/docs/runbooks/rollback-recovery.md`
- `packages/apps/proto-cook/infra/docs/runbooks/security-incident.md`
- `apps/proto-cook-web/README.md`
