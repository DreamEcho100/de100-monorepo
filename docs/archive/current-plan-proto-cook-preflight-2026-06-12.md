...
Current active phase: None - video-ready files roadmap complete; optional credentialed R2 smoke remains a follow-up.
...
| 13    | Provider-backed MinIO/R2/live media smoke                  | Done        |
...
1. Added repo-owned MinIO service to `docker-compose.yml`.
2. Added root scripts:
   - `pnpm files:minio:up`
   - `pnpm files:minio:smoke`
3. Added LMS API smoke script for public/private buckets, object round trips, multipart init/abort, encrypted-HLS generation, object upload/read, segment range read, and cleanup.
4. Started MinIO from the repo compose service.
5. Ran provider-backed MinIO smoke successfully against `http://127.0.0.1:9000`.
6. Validated:
   - public/private bucket creation
   - public/private object round trips
   - S3 multipart init/abort
   - local ffmpeg encrypted-HLS fixture generation
   - upload of original, manifest, AES key, and segment objects
   - manifest/key readback
   - segment range read
   - uploaded source object metadata
   - cleanup of smoke objects
7. Ran focused LMS API gates.
8. Ran global format/lint, typecheck, and test gates.

Follow-up, only when credentials/environment are available:

1. Repeat a narrow R2 smoke with real credentials.
2. Optionally add a long-video smoke using a larger fixture outside the default fast CI/local gate.
3. Optionally add a visible playback smoke using a real provider-backed HLS object set.

## 6. Post-Roadmap Follow-Ups

The video-ready files roadmap is complete. Future work should be opened as separate scoped phases only when the environment or product decision is ready:

1. Credentialed R2 production smoke:
   - use real R2 credentials
   - verify public/private buckets or equivalent prefixes
   - verify signed playback/session behavior against remote objects
   - document provider-specific CORS, URL, cache, and range-read behavior
2. Larger-course-video performance smoke:
   - use a fixture closer to course-video defaults
   - verify multipart upload, worker processing duration, artifact promotion, and cleanup
   - record storage, bandwidth, and processing timing evidence
3. Production player hardening:
   - replay real HLS sessions in headed browser
   - record QoE events
   - validate captions, poster, AES key retrieval, expiry, and entitlement failures
4. DRM/vendor decision phase:
   - keep signed HLS as default until real Shaka/DRM or managed-provider evidence changes the recommendation
   - compare Cloudflare Stream, Mux, Bunny, and self-owned R2/Shaka-style paths with operational cost included
...
178. DONE: Phase 13 setup added repo-owned MinIO compose service, root MinIO helper scripts, LMS API provider smoke script, and updated MinIO setup docs.
179. PASS: `pnpm files:minio:up` started the repo-owned MinIO compose service after Docker socket access was allowed.
180. PASS: `pnpm files:minio:smoke` against `http://127.0.0.1:9000`; run id `phase13-2026-06-12T05-34-09-434Z-d768b94d-6f89-4f3f-909b-d09ba9dd78d2`, six smoke objects uploaded, verified, and cleaned up.
181. PASS: `pnpm -F @de100/apps-lms-api format-and-lint:check`.
182. PASS: `pnpm -F @de100/apps-lms-api type:check`.
183. PASS: `pnpm -F @de100/apps-lms-api test`.
184. PASS: global `pnpm format-and-lint:check`.
185. PASS: global `pnpm type:check`.
186. PASS: global `pnpm test`.
187. DONE: Phase 13 closed. The video-ready files roadmap is complete; only optional credentialed R2 and larger media performance/playback smoke follow-ups remain.
188. PASS: final global `pnpm format-and-lint:check`, `pnpm type:check`, and `pnpm test` after Phase 13 closure notes, DX report updates, and final smoke-helper cleanup.

## 8. Phase 9 Completion Notes
